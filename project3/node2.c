...
void rtinit2() {
	...
	neighbor2 = getNeighborCosts(THIS_NODE);
	for (int i = 0; i < neighbor2->NodesInNetwork; i++) {
		dt2.costs[i][i] = initCost2[i] = mins2[i] = neighbor2->NodeCosts[i];
		printf("***** The cost of the node from %d to %d is %d.\n", THIS_NODE,
				i, neighbor2->NodeCosts[i]);
	}

	for (int i = 0; i < MAX_NODES; i++) {
		// Cases to skip: if self, or if not connected to
		if (i == THIS_NODE || initCost2[i] == INFINITY) {
			continue;
		}
...
void rtupdate2(struct RoutePacket *rcvdpkt) {
	...
	if (dt2.costs[origin_node][origin_node] > rcvdpkt->mincost[THIS_NODE]) { //if a shorter path is reported to node
		dt2.costs[origin_node][origin_node] = rcvdpkt->mincost[THIS_NODE]; //change path to shorter one
	...
	for (int i = 0; i < MAX_NODES; i++) {
		if (dt2.costs[i][origin_node]
				> rcvdpkt->mincost[i] + dt2.costs[origin_node][origin_node]) {
			dt2.costs[i][origin_node] = rcvdpkt->mincost[i]
					+ dt2.costs[origin_node][origin_node];
			flag = 1; // Flag for updated tables to be sent out
	...
	for (int i = 0; i < MAX_NODES; i++) {
		for (int j = 0; j < MAX_NODES; j++) {
			if (dt2.costs[i][j] < mins2[i]) {
				flag = 1; // Flag for updated tables to be sent out
				mins2[i] = dt2.costs[i][j];
			}
		}
	}
	...
	if (flag) {
		...
		printdt2(THIS_NODE, neighbor2, &dt2);
