...
void rtinit1() {
	...
	neighbor1 = getNeighborCosts(THIS_NODE);
	for (int i = 0; i < neighbor1->NodesInNetwork; i++) {
		dt1.costs[i][i] = initCost1[i] = mins1[i] = neighbor1->NodeCosts[i];
		printf("***** The cost of the node from %d to %d is %d.\n", THIS_NODE,
				i, neighbor1->NodeCosts[i]);
	}

	for (int i = 0; i < MAX_NODES; i++) {
		// Cases to skip: if self, or if not connected to
		if (i == THIS_NODE || initCost1[i] == INFINITY) {
			continue;
		}
...
void rtupdate1(struct RoutePacket *rcvdpkt) {
	...
	if (dt1.costs[origin_node][origin_node] > rcvdpkt->mincost[THIS_NODE]) { //if a shorter path is reported to node
		dt1.costs[origin_node][origin_node] = rcvdpkt->mincost[THIS_NODE]; //change path to shorter one
	...
	for (int i = 0; i < MAX_NODES; i++) {
		if (dt1.costs[i][origin_node]
				> rcvdpkt->mincost[i] + dt1.costs[origin_node][origin_node]) {
			dt1.costs[i][origin_node] = rcvdpkt->mincost[i]
					+ dt1.costs[origin_node][origin_node];
			flag = 1; // Flag for updated tables to be sent out
	...
	for (int i = 0; i < MAX_NODES; i++) {
		for (int j = 0; j < MAX_NODES; j++) {
			if (dt1.costs[i][j] < mins1[i]) {
				flag = 1; // Flag for updated tables to be sent out
				mins1[i] = dt1.costs[i][j];
			}
		}
	}
	...
	if (flag) {
		...
		printdt1(THIS_NODE, neighbor1, &dt1);
